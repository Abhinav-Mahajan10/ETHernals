...
class AppDir extends Component
{
  ...
  async loadBlockchainData() {
	...
	//   const form = await app.methods.form().call()
	//   this.setState({form})

	//   const MedForm = await app.methods.MedForm().call()
  ...
  constructor(props) {
    ...
    this.state = {
	  ...
	  form : new MedicalForm('unanswered', 'unanswered', 'unanswered'),
	  //MedForm : [fullname, mail, phno, gender, date, height, weight, blood, vacc, all, pres, sym, trav, his, ename, eno, ex]
	//   MedForm : ["Unanswered", "Unanswered", "Unanswered", "Unanswered", "Unanswered", "Unanswered", "Unanswered", 
	//   "Unanswered", "Unanswered", "Unanswered", "Unanswered", "Unanswered", "Unanswered", "Unanswered", "Unanswered", 
	...
	this.uploadMedicalForm = this.uploadMedicalForm.bind(this)
  }
  ...
  uploadForm = (name, mail, phno, gender, date, height, weight, blood, vacc, all, pres, sym, trav, his, ename, eno, ex) => {
	this.setState({loading : true})
	  this.state.app.methods.uploadForm(name, mail, phno, gender, date, height, weight, blood, vacc, all, pres, sym, trav, his, ename, eno, ex).send({ from: this.state.account }).on('transactionHash', (hash) => {
        this.setState({ loading: false })
      })
  }

  uploadMedicalForm = (name, mail, phno, gender, date, height, weight, blood, vacc, all, pres, sym, trav, his, ename, eno, ex) => {
	  this.setState({loading : true})
	  this.state.app.methods.uploadMedicalForm([name, mail, phno, gender, date, height, weight, blood, vacc, all, pres, sym, trav, his, ename, eno, ex]).send({ from: this.state.account }).on('transactionHash', (hash) => {
		this.setState({ loading: false })
	})
  }
    ...
    return (
            <div>
                <Routes>
					...
					MedForm={this.state.MedForm} 
					uploadMedicalForm={this.uploadMedicalForm}/> }/>
